...
bool NewKernelHandler::verifyAddress(uint64_t a) {
	bool result = false;
...
SIZE_T NewKernelHandler::pageEntryToProtection(SIZE_T entry) {
...
BOOL NewKernelHandler::GetNextReadablePageCR3(SIZE_T cr3, SIZE_T VirtualAddress, LPVOID ReadableVirtualAddress) {
	uint64_t pml4index;
	uint64_t pagedirptrindex;
	uint64_t pagedirindex;
								...
								while (pagedirindex < 512) {
									if ((pagedirpage[pagedirindex] & 1) == 1) {
										...
										if ((pagedirpage[pagedirindex] & (1 << 7)) == (1 << 7)) {
											...
											ReadableVirtualAddress = reinterpret_cast<void*>((pagedirindex << 21) + (pagedirptrindex << 30) + (pml4index << 39));
											return true;
										}
										else {
											...
											if (ReadPhysicalMemory(0, reinterpret_cast<void*>(x), pagetablepage, 4096, &x)) {
												while (pagetableindex < 512) {
													if ((pagetablepage[pagetableindex] & 1) == 1) {
														...
														ReadableVirtualAddress = reinterpret_cast<void*>((pagetableindex << 12) + (pagedirindex << 21) + (pagedirptrindex << 30) + (pml4index << 39));
														return true;
													}
													++pagetableindex;
												}
...
BOOL NewKernelHandler::GetPageInfoCR3(SIZE_T cr3, SIZE_T VirtualAddress, PMEMORY_BASIC_INFORMATION lpBuffer) {
	int pml4index;
	int pagedirptrindex;
	int pagedirindex;
	int pagetableindex;
...
BOOL NewKernelHandler::VirtualToPhysicalCR3(SIZE_T cr3, SIZE_T VirtualAddress, SIZE_T PhysicalAddress) {
	int pml4index;
	int pagedirptrindex;
	int pagedirindex;
	int pagetableindex;
...
SIZE_T NewKernelHandler::VirtualQueryExCR3(SIZE_T cr3, LPCVOID lpAddress, PMEMORY_BASIC_INFORMATION lpBuffer, SIZE_T dwLength) {
...
BOOL NewKernelHandler::ReadProcessMemoryCR3(SIZE_T cr3, LPCVOID lpBaseAddress, LPVOID lpBuffer, SIZE_T nSize, SIZE_T* lpNumberOfBytesRead) {
	...
	while (nSize > 0) {
		...
		uint64_t tempadd = (0x1000 - (currentAddress & 0xfff));
		blocksize = (nSize >= tempadd) ? tempadd : nSize;
...
BOOL NewKernelHandler::WriteProcessMemoryCR3(SIZE_T cr3, LPCVOID lpBaseAddress, LPCVOID lpBuffer, SIZE_T nSize, SIZE_T* lpNumberOfBytesWritten) {
	uint64_t currentAddress;
	uint64_t PA = { 0 };
	int i;
	uintptr_t x;
...
BOOL NewKernelHandler::WriteProcessMemory(HANDLE hProcess, LPCVOID lpBaseAddress, LPVOID lpBuffer, SIZE_T nSize, SIZE_T* lpNumberOfBytesWritten) {
	TWriteLogEntry* wle = new TWriteLogEntry();;
	uintptr_t x;
	uint64_t cr3;
	...
	if (wle->logWrites) {
		if (nSize < 64 * 1024 * 1024) {
			...
			ReadProcessMemory(hProcess, const_cast<LPVOID>(lpBaseAddress), wle->originalbytes, nSize, &x);
			wle->originalsize = x;
		}
	}
	...
	if (((reinterpret_cast<uint64_t>(lpBaseAddress) & (uint64_t(1) << 63)) != 0) && // kernelmode access
		...
		if (dbk32functions1->GetCR3(hProcess, &cr3)) {
			// todo: maybe just a getkernelcr3
	...
	if (result && wle->logWrites && wle != nullptr) {
		wle->newbytes = new BYTE[*lpNumberOfBytesWritten];
		ReadProcessMemory(hProcess, const_cast<void*>(lpBaseAddress), wle->newbytes, *lpNumberOfBytesWritten, &x);
		wle->newsize = x;
...
BOOL NewKernelHandler::ReadProcessMemory(HANDLE hProcess, LPVOID lpBaseAddress, LPVOID lpBuffer, SIZE_T nSize, SIZE_T* lpNumberOfBytesWritten) {
	...
	if (
		((reinterpret_cast<uint64_t>(lpBaseAddress) & (uint64_t(1) << 63)) != 0) && // kernelmode access
		(WriteProcessMemoryActual == defaultWPM) &&
		isRunningDBVM() || DBVMWatchBPActive) {
		if (dbk32functions1->GetCR3(hProcess, &cr3)) { // todo: maybe just a getkernelcr3
...
DWORD NewKernelHandler::VirtualQueryEx(HANDLE hProcess, LPCVOID lpAddress, PMEMORY_BASIC_INFORMATION lpBuffer, SIZE_T dwLength) {
	uint64_t cr3;
...
BOOL NewKernelHandler::Is64BitProcess(HANDLE processhandle)
{
	BOOL iswow64 = FALSE;
...
void NewKernelHandler::NeedsDBVM(const std::string& reason) {
	std::string r = reason;
...
BOOL NewKernelHandler::loaddbvmifneeded(const std::string& reason) {
...
BOOL NewKernelHandler::isDBVMCapable()
{
	int r[4];
	__cpuid(r, 1);
...
BOOL NewKernelHandler::hasEPTSupport() {
...
void NewKernelHandler::LoadDBK32()
{
	if (!DBKLoaded)
	{
		...
		dbk32functions1->DBK32Initialize();
		DBKLoaded = (dbk32functions1->hdevice != 0) && (dbk32functions1->hdevice != INVALID_HANDLE_VALUE);
		...
		// Add a wrapper function to convert the member function pointer to a regular function pointer



		// Assign the wrapper function to GetThreadsProcessOffset


		//GetThreadsProcessOffset = reinterpret_cast<TGetThreadsProcessOffset>(&dbk32functions::GetThreadsProcessOffset);
...
void NewKernelHandler::DBKFileAsMemory(const std::string& fn, uintptr_t* baseaddress)
{
